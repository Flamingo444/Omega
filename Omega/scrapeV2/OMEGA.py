...
def crawlNBATable(url,playerID):
    ...
    try:
        ...
        response = requests.get(
            url,
            proxies = {
                "http": "http://674ff84396704d699fe27f712120e913:@proxy.crawlera.com:8011/",
                "https": "http://674ff84396704d699fe27f712120e913:@proxy.crawlera.com:8011/",
            },
        ...
        counter = counter+1
        if ((counter % 25) == 0):
            print(Fore.WHITE+"["+Fore.MAGENTA+"*"+Fore.WHITE+"]"+Style.RESET_ALL+' ' + str(counter) + ' links parsed')
    ...
    with open(csv1, 'r') as file1, open(csv2, 'r') as file2:
        ...
        for row1 in reader1:
            if row1 not in file2_rows:
                print(f'Row only in file1: {row1}')

        for row2 in file2_rows:
            if row2 not in reader1:
                print(f'Row only in file2: {row2}')